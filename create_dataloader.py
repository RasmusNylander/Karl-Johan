...
class dataset (Dataset):
    def __init__(self,
                 ...
                 seed=42):        
        if train:
            self.image_paths = list(pd.read_csv(data_path+"/train.csv", names=["files"], header=0).files)
        else:
            self.image_paths = list(pd.read_csv(data_path+"/test.csv", names=["files"], header=0).files)
...
def make_dataloaders(batch_size = 16, seed = 42, data_path = '../datasets/sorted_downscaled'):
    """
