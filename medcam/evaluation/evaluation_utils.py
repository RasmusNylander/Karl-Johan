...
def _preprocessing(attention_map, mask, attention_threshold):
    ...
    if (
        np.sum(attention_map < 0) > 0
    ):  # For gbp and ggcam as they contain negative values, which would otherwise falsify the evaluation
...
def _intersection_over_union(
    binary_attention_map, mask, weights
):  # TODO: wiou is bad and wrong, maybe not even possible?
