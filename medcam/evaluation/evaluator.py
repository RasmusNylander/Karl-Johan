...
class Evaluator:
    def __init__(self, save_path, metric="wioa", threshold="otsu", layer_ordering=None):
        ...
        self.scores = pd.DataFrame(
            columns=["evaluation", "layer", "class_label", "name"]
        )
    ...
    def comp_score(self, attention_map, mask, layer=None, class_label=None, name=None):
        ...
        """
        score = evaluation_utils.comp_score(
            attention_map, mask, self.metric, self.threshold
        )
    ...
    def _add(self, score, layer=None, class_label=None, name=None):
        ...
        self.scores = self.scores.append(
            {
                "evaluation": score,
                "layer": layer,
    ...
    def _comp_means(self, scores):
        ...
        if self.layer_ordering is not None and unique_layers[0] != "":
            unique_layers = sorted(
                set(self.layer_ordering).intersection(unique_layers),
                key=lambda x: self.layer_ordering.index(x),
            )
        for unique_layer in unique_layers:
            ...
            for unique_class_label in unique_class_labels:
                ...
                mean_scores = mean_scores.append(
                    {
                        "mean_score": mean_score,
                        "layer": unique_layer,
                        "class_label": unique_class_label,
                    },
                    ignore_index=True,
                )
    ...
    def _reduce(self, layers, mean_scores):
        ...
        saved = [
            [layers[0], mean_scores[0]],
            [layers[len(layers) - 1], mean_scores[len(layers) - 1]],
        ]
        i = 0
        while i < len(layers):
            j = 0
