...
def inject(
    ...
    if evaluate:
        medcam_dict["Evaluator"] = Evaluator(
            ...
            metric=metric,
            threshold=threshold,
            layer_ordering=medcam_utils.get_layers(model),
        )
    ...
    model_backend, heatmap = _assign_backend(
        backend, model, layer, None, retain_graph
    )  # TODO: Remove postprocessor in a later version
...
def save_attention_map(self, attention_map, raw_input=None):
    ...
    medcam_utils.save_attention_map(
        filename=self.medcam_dict["output_dir"]
        + "/"
...
def dump(self):
    ...
    if self.medcam_dict["save_pickle"]:
        with open(
            self.medcam_dict["output_dir"] + "/attention_maps.pkl", "wb"
        ) as handle:  # TODO: Save every 1GB
            pickle.dump(
                self.medcam_dict["pickle_maps"],
                handle,
    ...
    if self.medcam_dict["enabled"]:
        with torch.enable_grad():
            ...
            output, attention_map, batch_size, channels, data_shape = self.medcam_dict[
                "model_backend"
            ].generate_attention_map(batch, label)
            if attention_map:
                if len(attention_map.keys()) == 1:
                    self.medcam_dict["current_attention_map"] = attention_map[
                        list(attention_map.keys())[0]
                    ]
    ...
    if batch is not None and not self.medcam_dict["tested"]:
        ...
        if not internal:
            ...
            if self.medcam_dict["output_dir"] is not None:
                np.savetxt(
                    self.medcam_dict["output_dir"] + "/registered_layers.txt",
                    np.asarray(registered_hooks).astype(str),
                    fmt="%s",
                )
    ...
    if backend == "gbp":
        return (
            GuidedBackPropagation(
                model=model, postprocessor=postprocessor, retain_graph=retain_graph
            ),
            False,
        )
    elif backend == "gcam":
        ...
        for j in range(batch_size):
            for k in range(channels):
                ...
                self._save_attention_map(
                    attention_map_single, layer_output_dir, j, k, raw_input_single
                )
                if self.medcam_dict["evaluate"]:
                    if mask is None:
                        raise ValueError("Mask cannot be none in evaluation mode")
                    score = self.medcam_dict["Evaluator"].comp_score(
                        attention_map_single,
                        mask[j][k].squeeze(),
                        layer=layer_name,
                        class_label=k,
                    )
...
def _replace_or_return(self, output, attention_map, data_shape):
    ...
    if self.medcam_dict["_replace_output"] or self.medcam_dict["return_attention"]:
        if len(attention_map.keys()) == 1:
            output_attention_map = torch.FloatTensor(
                self.medcam_dict["current_attention_map"]
            ).to(str(self.medcam_dict["device"]))
            if (
                data_shape is not None
            ):  # If data_shape is None then the task is classification -> return unchanged attention map
                output_attention_map = medcam_utils.interpolate(
                    output_attention_map, data_shape
                )
            if self.medcam_dict["_replace_output"]:
