...
class GuidedGradCam:
    ...
    def __init__(
        ...
        self.model_GCAM = GradCAM(
            model=model,
            target_layers=target_layers,
            postprocessor=postprocessor,
            retain_graph=retain_graph,
        )
        self.model_GBP = GuidedBackPropagation(
    ...
    def generate_attention_map(self, batch, label):
        ...
        (
            output,
            self.attention_map_GCAM,
            output_batch_size,
            output_channels,
            output_shape,
        ) = self.model_GCAM.generate_attention_map(batch.clone(), label)
    ...
    def generate(self):  # TODO: Redo ggcam, find a solution for normalize_per_channel
        ...
        for layer_name in self.attention_map_GCAM.keys():
            if (
                self.attention_map_GBP.shape
                == self.attention_map_GCAM[layer_name].shape
            ):
                self.attention_map_GCAM[layer_name] = np.multiply(
                    self.attention_map_GCAM[layer_name], self.attention_map_GBP
                )
            else:
    ...
    def _generate_gbp(self, batch, label):
        ...
        attention_map = attention_map.repeat(
            1,
            self.model_GBP.output_channels,
