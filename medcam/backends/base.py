...
class _BaseWrapper:
    ...
    def _extract_metadata(
        self, input, output
    ):  # TODO: Does not work for classification output (shape: (1, 1000)), merge with the one in medcam_inject
        ...
        if self.model.medcam_dict["data_shape"] == "default":
            if (
                len(output.shape) == 2
            ):  # Classification -> Cannot convert attention map to classifiaction
    ...
    def _set_postprocessor_and_label(self, output):
        if self.model.medcam_dict["label"] is None:
            if (
                output.shape[0] == self.output_batch_size and len(output.shape) == 2
            ):  # classification
