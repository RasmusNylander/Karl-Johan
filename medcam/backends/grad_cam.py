...
class GradCAM(_BaseWrapper):
    def __init__(
        self, model, target_layers=None, postprocessor=None, retain_graph=False
    ):
        """
        ...
        super(GradCAM, self).__init__(
            model, postprocessor=postprocessor, retain_graph=retain_graph
        )
    ...
    def _register_hooks(self):
        ...
        for name, module in self.model.named_modules():
            if self.target_layers is None or name in self.target_layers:
                ...
                self.forward_handlers.append(
                    module.register_forward_hook(forward_hook(name))
                )
                if ENABLE_MODULE_HOOK:
                    self.backward_handlers.append(
    ...
    def _auto_layer_selection(self):
        ...
        for layer in module_names:
            try:
                ...
                if not isinstance(fmaps, torch.Tensor) or not isinstance(
                    grads, torch.Tensor
                ):
                    continue
                if (
                    len(fmaps.shape) == 4
                    and len(grads.shape) == 4
                    and fmaps.shape[2] > 1
                    and fmaps.shape[3] > 1
                    and grads.shape[2] > 1
                    and grads.shape[3] > 1
                    ...
                    if is_grads_zero:
                        print(
                            "WARNING: All gradients in layer {} are zero! The attention map will be empty.".format(
                                layer
                            )
                        )
                    break
    ...
    def _generate_helper(self, fmaps, grads, layer):
        ...
        try:
            ...
            raise RuntimeError(
                "Number of set channels ({}) is not a multiple of the feature map channels ({}) in layer: {}".format(
                    self.output_channels, fmaps.shape[1], layer
                )
            )
    ...
    def _check_hooks(self, layer):
        ...
        if not self.registered_hooks[layer][0] and not self.registered_hooks[layer][1]:
            raise ValueError(
                "Neither forward hook nor backward hook did register to layer: "
                + str(layer)
            )
            ...
            raise ValueError(
                "Backward hook did not register to layer: "
                + str(layer)
                + ", Check if the hook was registered to a layer that is skipped during backward and thus no gradients are computed"
            )
