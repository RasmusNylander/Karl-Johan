...
def save_attention_map(filename, attention_map, heatmap, raw_input):
    ...
    attention_map = normalize(attention_map.astype(np.float64))
...
def _resize_attention_map(attention_map, min_shape):
    ...
    if min(min_shape) < min(attention_map_shape):
        ...
        data_shape = (
            attention_map_shape[0] * resize_factor,
            attention_map_shape[1] * resize_factor,
        )
...
def get_layers(model, reverse=False):
    ...
    while True:
        if index == len(layer_names) - 1:
            break
        if (
            sub_index < len(layer_names) - 1
            and layer_names[index]
            == layer_names[sub_index + 1][: len(layer_names[index])]
        ):
            sub_index += 1
...
def _interpolate_tensor(data, shape, squeeze):
    ...
    Shape needs to be as size (int or Tuple[int] or Tuple[int, int] or Tuple[int, int, int])"""
    _squeeze = 0
    if (len(shape) == 2 and len(data.shape) == 2) or (
        (len(shape) == 3 and len(data.shape) == 3)
    ):  # Add batch and channel dim
        data = data.unsqueeze(0).unsqueeze(0)
        _squeeze = 2
    elif (len(shape) == 2 and len(data.shape) == 3) or (
        (len(shape) == 3 and len(data.shape) == 4)
    ):  # Add batch dim
...
def overlay(raw_input, attention_map):
    ...
    attention_map = (
        attention_map.astype(np.float64) + raw_input.astype(np.float64)
    ) / 2
    attention_map *= 255
    return attention_map
